...
export default function UserWallet({ isOpen, onClose, onSelect }) {
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            await saveUserToWallet({ name: addForm.name, publicKey: pubHex, encryptedPrivateKey: ciphertext, iv, salt })
            await loadUsers()
            setView('select')
        } catch (err) {
            console.error(err)
    ...
    const handleActionSubmit = async (e) => {
        ...
        try {
            const decryptedPrivKey = await decryptKey(
                selectedUser.encryptedPrivateKey,
                selectedUser.iv,
                selectedUser.salt,
                actionForm.password
            )
            if (!decryptedPrivKey) throw new Error('Decryption returned empty.')
            ...
            } else if (actionForm.actionType === 'decryptGroupKey') {
                // ─── NEW BRANCH ───────────────────────────────────────────────
                // Strip 0x prefix and load private key as Buffer
                        ...
                        !selectedUser ? (
                            ...
                            <div className="action-panel">
                                ...
                                <form className="form" onSubmit={handleActionSubmit}>
                                    ...
                                    {/* NEW: encrypted group key input */}
                                    {actionForm.actionType === 'decryptGroupKey' && (
                                        <div className="form-group" style={{ animation: 'modal-up 0.2s ease' }}>
                                            <label className="form-label">Encrypted Group Key (Base64)</label>
                                            <textarea
                                                className="form-input" required rows={4}
                                                value={actionForm.encryptedGroupKey}
                                                onChange={e => setActionForm({ ...actionForm, encryptedGroupKey: e.target.value })}
                                                placeholder="Paste Base64-encoded ECIES ciphertext..."
                                                style={{ fontFamily: "'Courier New', monospace", fontSize: '0.78rem', resize: 'vertical' }}
                                            />
                                        </div>
                                    )}
